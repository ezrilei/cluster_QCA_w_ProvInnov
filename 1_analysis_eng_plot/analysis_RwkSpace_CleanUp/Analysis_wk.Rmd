...
font_add(
  ...
  bold    = "./fonts/Times New Roman Bold.ttf"   # 添加 bold
)
...
p_Y_X1_all <- ggplot(data = df.Fuzzy.1, aes(x = X1, y = Y)) + 
  ...
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "red") +  # y = x 线
  ggtitle("Cases: Y / X1") +
  xlab("set of cases with high R&D Investment (X1)") +
  ylab("set of cases with high Regional Innovation Capability (Y)") +
  xlim(range(c(df.Fuzzy.1$X1, df.Fuzzy.1$Y))) +  # 确保X轴范围与Y轴相同
  ylim(range(c(df.Fuzzy.1$X1, df.Fuzzy.1$Y))) +  # 确保Y轴范围与X轴相同
  ...
  theme(plot.title = element_text(
    ...
    plot.margin = unit(c(0.5, 0.5, 0.5, 0.5), "cm") # 调整图形的边距
  )  
...
p_negY_negX5_all <- ggplot(data = df.Fuzzy.3, aes(x = negX5, y = negY)) + 
  ...
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "red") +  # y = x 线
  ggtitle("Cases: ~Y / ~X5") +
  xlab("set of cases with low Makerspace Services (~X5)") +
  ylab("set of cases with low Regional Innovation Capability (~Y)") +
  xlim(range(c(df.Fuzzy.3$negX5, df.Fuzzy.3$negY))) +  # 确保X轴范围与Y轴相同
  ylim(range(c(df.Fuzzy.3$negX5, df.Fuzzy.3$negY))) +  # 确保Y轴范围与X轴相同
  theme_bw(base_family = "Times_New_Roman") +
  theme(plot.title = element_text(
...
p_negY_negX7_all <- ggplot(data = df.Fuzzy.2, aes(x = negX7, y = negY)) + 
  ...
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "red") +  # y = x 线
  ggtitle("Cases: ~Y / ~X7") +
  xlab("set of cases with low Cultivation of S&T Personnel (~X7)") +
  ylab("set of cases with high Regional Innovation Capability (~Y)") +
  xlim(range(c(df.Fuzzy.2$negX7, df.Fuzzy.2$negY))) +  # 确保X轴范围与Y轴相同
  ylim(range(c(df.Fuzzy.2$negX7, df.Fuzzy.2$negY))) +  # 确保Y轴范围与X轴相同
  theme_bw(base_family = "Times_New_Roman") +
...
# store to pdf (create a pdf device) 
pdf(file = "./output/eng-【Analysis_wk】2.必要性分析_汇总一致性大于0.9_且_汇总覆盖度大于0.5★.pdf", 
    width = 12, height = 4
    )
...
p_BECON_AdjDis_greater_0_2_Consistencies <- ggplot(
  ...
  theme_bw(base_family = "SongTi") + # 默认字体为 Times
  
  theme(
    axis.title = element_text(family = "SongTi", size = 14),   # 坐标轴 title 用仿宋
    legend.title = element_text(family = "SongTi", size = 13), # 图例   title 用仿宋
...
#### draw BECONS_AdjDis > 0.2 & config with BECONS > 0.9 & BECOV > 0.5
...
# 只保留 存在至少一个 BECONS > 0.9 且 BECOV > 0.5 的 Config （不然 图中线条太多了 看不过来）
wide_yearly_BECON_AdjDis_greater_0_2_config2 <- wide_yearly_BECON_AdjDis_greater_0_2_config %>%
  filter(Config %in% c("Y / X3", "Y / X6", "~Y / ~X3", "~Y / ~X6"))

# 确保排序正确

label_data <- wide_yearly_BECON_AdjDis_greater_0_2_config2 %>%
  group_by(Config) %>%
  filter(YEAR == max(YEAR)) %>%
  ungroup()
...
# store to pdf (create a pdf device) ---------------
# pdf(file = "./output_manual/组间一致性水平趋势图.pdf", width = 10, height = 8)
pdf(file = "./output/【Analysis_wk】2.必要条件分析_组间一致性调整距离大于0.2且存在组间一致性大于0.9、组间覆盖度大于0.5的因果组合的_组间一致性变化趋势图.pdf", width = 10, height = 8)


p_BECON_AdjDis_greater_0_2_Consistencies2 <- ggplot(
  data = wide_yearly_BECON_AdjDis_greater_0_2_config2, 
  aes(x = YEAR, y = Consistencies, group = Config, color = Config)
) +
  geom_line() +
  geom_point() +
...
p_BECON_AdjDis_greater_0_2_Consistencies2
...
# print to viewer in Rstudio、
print(p_BECON_AdjDis_greater_0_2_Consistencies2)
...
## `Y / X3` 2016年 ----
df.Fuzzy.4_suppl <- df.Fuzzy %>% 
  filter(YEAR == 2016)

# 创建散点图对象，并添加从原点出发的斜对角线

# p_Y_X3_2016 - 为典型的trivialness BECON = 1.000；BECOV=0.476 
# p_Y_X3_2016 <- ggplot(data = df.Fuzzy.4_suppl, aes(x = X3, y = Y)) + 
#   geom_point() +
#   geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "red") +  # y = x 线
#   ggtitle("情况 Y / X3（2016年）") +
#   xlab("X3值") +
#   ylab("Y值") +
#   xlim(range(c(df.Fuzzy.4_suppl$X3, df.Fuzzy.4_suppl$Y))) +  # 确保X轴范围与Y轴相同
#   ylim(range(c(df.Fuzzy.4_suppl$X3, df.Fuzzy.4_suppl$Y))) +  # 确保Y轴范围与X轴相同
#   theme_bw() +
#   theme(plot.title = element_text(hjust = 0.5),  # 标题居中
#         plot.margin = unit(c(0.5, 0.5, 0.5, 0.5), "cm")
#         )  # 调整图形的边距
...
# 创建散点图对象，并添加从原点出发的斜对角线
p_Y_X3_2017 <- ggplot(data = df.Fuzzy.4, aes(x = X3, y = Y)) + 
  geom_point() +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "red") +  # y = x 线
  ggtitle("情况 Y / X3（2017年）") +
  xlab("X3值") +
  ylab("Y值") +
  xlim(range(c(df.Fuzzy.4$X3, df.Fuzzy.4$Y))) +  # 确保X轴范围与Y轴相同
  ylim(range(c(df.Fuzzy.4$X3, df.Fuzzy.4$Y))) +  # 确保Y轴范围与X轴相同
  theme_bw() +
  theme(plot.title = element_text(hjust = 0.5),  # 标题居中
        plot.margin = unit(c(0.5, 0.5, 0.5, 0.5), "cm")
        )  # 调整图形的边距
...
p_Y_X3_2017 <- ggplot(data = df.Fuzzy.4, aes(x = X3, y = Y)) + 
  geom_point() +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "red") +  # y = x 线
  ggtitle("Cases in 2017: Y / X3") +
  xlab("set of cases with high Government Innovation Subsidies (X3)") +
  ylab("set of cases with high Regional Innovation Capability (Y)") +
  xlim(range(c(0, 1))) +  # 确保X轴范围与Y轴相同
  ylim(range(c(0, 1))) +  # 确保Y轴范围与X轴相同
  theme_bw(base_family = "Times_New_Roman") +
  theme(plot.title = element_text(
    family = "Times_New_Roman",
    face   = "bold",      # 标题加粗
...
p_Y_X6_2021 <- ggplot(data = df.Fuzzy.5, aes(x = X6, y = Y)) + 
  geom_point() +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "red") +  # y = x 线
  ggtitle("Cases in 2021: Y / X6") +
  xlab("set of cases with high Government Innovation Procurement (X6)") +
  ylab("set of cases with high Regional Innovation Capability (Y)") +
  xlim(range(c(0, 1))) +  # 确保X轴范围与Y轴相同
  ylim(range(c(0, 1))) +  # 确保Y轴范围与X轴相同
  theme_bw(base_family = "Times_New_Roman") +
...
p_negY_negX3_2019 <- ggplot(data = df.Fuzzy.8, aes(x = negX3, y = negY)) + 
  geom_point() +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "red") +  # y = x 线
  ggtitle("Cases in 2019: ~Y / ~X3") +
  xlab("set of cases with low Government Innovation Subsidies (~X3)") +
  ylab("set of cases with low Regional Innovation Capability (~Y)") +
  xlim(range(c(0, 1))) +  # 确保X轴范围与Y轴相同
  ylim(range(c(0, 1))) +  # 确保Y轴范围与X轴相同
  theme_bw(base_family = "Times_New_Roman") +
...
p_negY_negX6_2016 <- ggplot(data = df.Fuzzy.13, aes(x = negX6, y = negY)) + 
  geom_point() +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "red") +  # y = x 线
  ggtitle("Cases in 2016: ~Y / ~X6") +
  xlab("set of cases with low Government Innovation Procurement (~X6)") +
  ylab("set of cases with low Regional Innovation Capability (~Y)") +
  xlim(range(c(0, 1))) +  # 确保X轴范围与Y轴相同
  ylim(range(c(0, 1))) +  # 确保Y轴范围与X轴相同
  theme_bw(base_family = "Times_New_Roman") +
...
# store to pdf (create a pdf device) 
# pdf(file = "./output/eng-【Analysis_wk】2.必要条件分析_组间一致性调整距离大于0.2的因果组合_汇总一致性大于0.9_且_汇总覆盖度大于0.5'.pdf", 
#     width = 12, height = 16 # 太挤了 axis title 显示不全
# )
...
```{r}
tt_test <- QCA::truthTable(data = df.Fuzzy.case, 
                        outcome = "Y", 
                        conditions = conditions, 
                        incl.cut = 0.5, pri.cut = 0.5#,
                        #complete = TRUE 
                        #show.cases = TRUE,
                        #sort.by = "OUT"
                        )
...
tt_test_TruthTable <- tt_test$tt



```

#### 可视化 natural breaking point <---------
...
# 提取 PRI 列（去除 NA）----
pri_values <- na.omit(tt_test_TruthTable$PRI)

# 可视化 PRI 的分布 + 密度曲线 + 自然断点参考线
ggplot(data = data.frame(PRI = pri_values), aes(x = PRI)) +
  geom_histogram(aes(y = after_stat(density)),
                 bins = 30,
                 fill = "#69b3a2",
                 alpha = 0.7,
                 color = "white") +
  geom_density(color = "#1f78b4", size = 1.2, adjust = 1) +
  geom_vline(xintercept = 0.6, 
             linetype = "dashed", 
             color = "red", 
             linewidth = 1) +
  labs(
    title = "PRI 值分布及自然断点可视化",
    x = "PRI（Proportional Reduction in Inconsistency）",
    y = "密度"
  ) +
  theme_minimal(base_size = 13)

# 提取 incl 列（去除 NA）----
incl_values <- na.omit(tt_test_TruthTable$incl)

# 可视化 incl 的分布 + 密度曲线 + 自然断点参考线
ggplot(data = data.frame(incl = incl_values), aes(x = incl)) +
  geom_histogram(aes(y = after_stat(density)),
                 bins = 30,
                 fill = "#69b3a2",
                 alpha = 0.7,
...
# 提取 n 列（去除 NA）----
n_values <- na.omit(tt_test_TruthTable$n)

# 可视化 incl 的分布 + 密度曲线 + 自然断点参考线
ggplot(data = data.frame(n = n_values), aes(x = n)) +
  geom_histogram(aes(y = after_stat(density)),
                 bins = 30,
                 fill = "#69b3a2",
                 alpha = 0.7,
...
df_cluster_data_allY_BECONS_BECOV <- df_cluster_data_allY_BECONS_BECOV %>%
  mutate(
    PATHWAY = case_when(
...
df_cluster_data_allY_BECONS_BECOV_eng <- df_cluster_data_allY_BECONS_BECOV %>%
  mutate(
    PATHWAY = case_when(
      CONFIG == "X1*X3*X4*X5*X7*X8*X9 → Y" ~ "1a",
      CONFIG == "X1*X4*X5*X6*X7*X8*X9 → Y" ~ "1b",
      CONFIG == "X1*X2*X3*X4*X5*~X6*X7*X8 → Y" ~ "2",
      CONFIG == "~X1*~X3*~X4*~X5*~X7*~X8*~X9 → ~Y" ~ "3"
    )
  )

print(df_cluster_data_allY_BECONS_BECOV)
print(df_cluster_data_allY_BECONS_BECOV_eng)
```
...
# text repel data
label_data_eng <- df_cluster_data_allY_BECONS_BECOV_eng %>%
  group_by(PATHWAY) %>%
  filter(YEAR == max(YEAR)) %>%
  ungroup()
...
# store to pdf (create a pdf device) ---------------
# pdf(file = "./output_manual/组间一致性水平趋势图.pdf", width = 10, height = 8)
pdf(file = "./output/eng-【Analysis_wk】3.充分性分析_组间一致性变化趋势图.pdf", width = 10, height = 8)

p_config_BECONS_eng <- ggplot(df_cluster_data_allY_BECONS_BECOV_eng, 
                              aes(x = YEAR, y = BECONS, group = PATHWAY, color = PATHWAY)) +
  geom_line() +
  geom_point() +
  # geom_hline(yintercept = 0.90, linetype = "dashed", color = "black") +
  ...
  theme_bw(base_family = "Times_New_Roman") +
  theme(
    axis.title = element_text(family = "Times_New_Roman", size = 14),
    legend.title = element_text(family = "Times_New_Roman", face   = "bold", size = 13), # 图例标题加粗
    plot.margin = unit(c(1, 4, 1, 1), "cm"),
    legend.position = "bottom"
  ) +
  # guides(color = guide_legend(ncol = 1))  #+ # <<<<<<<< 每列 1 项，竖排
...
# text repel data
label_data <- df_cluster_data_allY_BECONS_BECOV_eng %>%
  group_by(PATHWAY) %>%
  filter(YEAR == max(YEAR)) %>%
  ungroup()
...
# store to pdf (create a pdf device) ---------------
pdf(file = "./output/eng-【Analysis_wk】3.充分性分析_组间覆盖度变化趋势图.pdf", width = 10, height = 8)

p_config_BECOV_eng <- ggplot(df_cluster_data_allY_BECONS_BECOV_eng, 
                             aes(x = YEAR, y = BECOV, group = PATHWAY, color = PATHWAY)) +
  geom_line() +
  geom_point() +
  geom_hline(yintercept = 0.50, linetype = "dashed", color = "black", alpha = 0.4) +
...
# 输出到 PDF 文件
 pdf(file = "./output/【Analysis_wk】组间一致性与覆盖度_趋势图_patch.pdf", width = 10, height = 8)
...
# 输出到 PDF 文件
 pdf(file = "./output/eng-【Analysis_wk】组间一致性与覆盖度_趋势图_patch★.pdf", width = 10, height = 8)

p_config_BECONS_eng / p_config_BECOV_eng +
  plot_layout(guides = "collect",
              heights = c(1,2)) & 
  theme(legend.position = "bottom") 
...
### def visualization - PIMS -patch - function
...
plot_PIMS_patch <- function(Region_Target) {
  ...
  p_case_pims <-
    ...
    theme(
      ...
      # 图例标题
      legend.text = element_text(family = "SongTi", size = 12),# <<<<<<<<<<<<<<<<
      axis.text.x = element_text(size = 12, face = "bold", family = "SongTi"),  # x轴：年份# <<<<<<<<<<<<<<<<
      axis.text.y = element_text(size = 12, face = "bold", family = "SongTi"),  # y轴：数值# <<<<<<<<<<<<<<<<
      # 图例标签
      legend.position = "bottom",
      plot.margin = unit(c(1, 4, 1, 1), "cm")
    ) +
  ...
  p_case_pims_radar <- ggradar(
    ...
    labs(title = paste0("路径隶属度演化（", region_target, "）")) +
    #theme_bw(base_family = "TimesNewRoman") +
    theme(
      text = element_text(family = "SongTi"),
      axis.title = element_text(family = "SongTi", size = 14),
  ...
  #       theme = theme(
  #         legend.position = "bottom",
  #         plot.title = element_text(size = 16, face = "bold", family = "SongTi"),
  #         plot.tag = element_text(size = 16, face = "bold", family = "SongTi")
  #         )
  #       )
  #   )
...
plot_PIMS_patch <- function(Region_Target) {
  ...
  p_case_pims <-
    ...
    scale_x_continuous(breaks = sort(unique(df_pims_region_long$YEAR))) +
    labs(
      # title = paste0("Evolution of Configuration Path Membership in ", region_target, " (2016–2023)"),
      # Evolution of Configurational Path Membership in Beijing (2016–2023)

      #subtitle = "每条线代表一个路径组态的 fuzzy membership 值",
      x = "year",
      y = "fuzzy-set membership scores",
      color = "Configuration Path"
    ) +
    ...
    theme(
      text = element_text(family = "SongTi"),
      plot.title = element_text(family = "Times_New_Roman", 
                                face   = "bold",      # 标题加粗
                                hjust  = 0.5          # 标题居中
                                ),
      axis.title = element_text(family = "Times_New_Roman", size = 14),
      axis.text = element_text(family = "Times_New_Roman"), 
      legend.title = element_text(family = "Times_New_Roman", face = "bold", size = 12), # 图例标题加粗
  ...
  p_case_pims_radar <- ggradar(
    ...
    theme(
      ...
      axis.title = element_text(family = "TimesNewRoman", size = 14),
      legend.title = element_text(family = "TimesNewRoman", size = 12),
      # 图例标题
      legend.text = element_text(family = "TimesNewRoman", size = 10),
      # 图例标签
      legend.position = "none", # 不显示 legend
      plot.title = element_blank() # 不显示 plot title
    )
  ...
  pdf(file = paste0("./output/eng-【Analysis_wk】3.充分性分析_路径模糊集隶属度_patch（", 
                    region_target, 
                    "）.pdf"),
      width = 14.5,
      height = 7)
  ...
  print(
    (p1 | p2) +
      ...
      plot_annotation(
        title = paste0(
          "Evolution of Configuration Path Membership in ",
          region_map2eng[region_target], # 中文转英文名称
          " (2016–2023)"
          ),
      theme = theme(
        plot.title = element_text(
          ...
          face = "bold",
          family = "Times_New_Roman",
          margin = margin(b = 15) # 标题和下面的图之间留15个pt的间距
        )
      )
    ) &
      theme(legend.position = "bottom")
...
### def function for PIMS patch plot - take vector input
...
# 稳健性检验 by adjust n、incl、PRI cut -----

## 创建目录 ./output/robustness 如果不存在
```{r}
dir_path_robust <- "./output/robustness"
if (!dir.exists(dir_path_robust)) {
  dir.create(dir_path_robust, recursive = TRUE)
}
```
...
get_intsol_Y_res_Final <- function(val.incl.cut, val.n.cut, val.pri.cut){
    tt_Y <- QCA::truthTable(data = df.Fuzzy.case, 
                          outcome = "Y", 
                          conditions = conditions, 
                          incl.cut = val.incl.cut, n.cut = val.n.cut, pri.cut = val.pri.cut, 
                          show.cases = TRUE,
                          sort.by = "OUT"
                          )
  ...
  writexl::write_xlsx(x = intsol_Y_res_Final,
                      path = paste0("./output/robustness/【Analysis_wk】3.充分条件分析_结果_Y", 
                                    "_incl.cut = ", val.incl.cut, 
                                    "_n.cut = ", val.n.cut, 
                                    "_pri.cut =", val.pri.cut,
                                    ".xlsx"
                                    )
...
} # end def funciton get_intsol_Y_res_Final
...
## def get_intsol_negY_res_Final

```{r}
get_intsol_negY_res_Final <- function(val.incl.cut, val.n.cut, val.pri.cut){
  ### 构建真值表 ----
  ...
  writexl::write_xlsx(x = intsol_negY_res_Final,
                      path = paste0("./output/robustness/【Analysis_wk】3.充分条件分析_结果_negY", 
                                    "_incl.cut = ", val.incl.cut, 
                                    "_n.cut = ", val.n.cut, 
                                    "_pri.cut =", val.pri.cut,
                                    ".xlsx"
...
} # end def funciton get_intsol_negY_res_Final
...
# 定义参数组合（±0.10范围内扰动）
params <- list(
  list(name = "Baseline", incl = 0.75, pri = 0.60, n = 2),
  
  list(name = "InclUp1",  incl = 0.80, pri = 0.60, n = 2),
  list(name = "InclUp2",  incl = 0.85, pri = 0.60, n = 2),
  list(name = "InclUp3",  incl = 0.90, pri = 0.60, n = 2),
  
  list(name = "PRIUp1",   incl = 0.75, pri = 0.65, n = 2),
  list(name = "PRIUp2",   incl = 0.75, pri = 0.70, n = 2),
  list(name = "PRIUp3",   incl = 0.75, pri = 0.75, n = 2),
  
  list(name = "nUp1",   incl = 0.75, pri = 0.60, n = 3),
  list(name = "nUp2",   incl = 0.75, pri = 0.65, n = 4),
  list(name = "nUp3",   incl = 0.75, pri = 0.70, n = 5)
)

lapply(params, function(param) {
  
  message("Running: ", param$name)
  
  # 正向路径分析
  get_intsol_Y_res_Final(val.incl.cut = param$incl, 
                         val.pri.cut = param$pri, 
                         val.n.cut = param$n)
  # 反向路径分析
  get_intsol_negY_res_Final(val.incl.cut = param$incl, 
                            val.pri.cut = param$pri, 
                            val.n.cut = param$n)
})
...
<!-- ### take 1  -->
<!-- ```{r} -->
<!-- get_intsol_Y_res_Final(val.incl.cut = 0.75,  -->
<!--                        val.n.cut = 2,  -->
<!--                        val.pri.cut = 0.6) -->

<!-- get_intsol_negY_res_Final(val.incl.cut = 0.75,  -->
<!--                           val.n.cut = 2,  -->
<!--                           val.pri.cut = 0.6) -->
<!-- ``` -->
...
<!-- ### take 2 -->
<!-- ```{r} -->
<!-- get_intsol_Y_res_Final(val.incl.cut = 0.85,  -->
<!--                        val.n.cut = 2,  -->
<!--                        val.pri.cut = 0.80) -->

<!-- get_intsol_negY_res_Final(val.incl.cut = 0.85,  -->
<!--                           val.n.cut = 2,  -->
<!--                           val.pri.cut = 0.80) -->
<!-- ``` -->
...
<!-- ### take 3 -->
<!-- ```{r} -->
<!-- get_intsol_Y_res_Final(val.incl.cut = 0.80,  -->
<!--                        val.n.cut = 2,  -->
<!--                        val.pri.cut = 0.75) -->

<!-- get_intsol_negY_res_Final(val.incl.cut = 0.80,  -->
<!--                           val.n.cut = 2,  -->
<!--                           val.pri.cut = 0.75) -->
<!-- ``` -->

### take an extrem value, i.e. n cut = 1, just to prove the function working just fine.
